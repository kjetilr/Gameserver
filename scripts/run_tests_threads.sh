 ...
 MAX_CLIENTS=700		# Maximum number of clients
 COUNTER=680
 ...
 # Perform run for both single-threaded and multi-threaded version
     for threadnum in {2..256..2};
     do
     echo "Running for $threadnum threads.   "
#	 if [ $i -eq 1 ]
#	 then
...
#	 fi
 ...
 # Set server options for this run
	 SERVER_OPTS="$SERVER_PORT $threadnum $SINGLETHREADED $LOAD_INTENSITY $LOAD_VECTOR_SIZE $NUM_SELECTORS $DURATION"
	 echo "SERVER_OPTS: $SERVER_OPTS"
 # Set client options for this run
	 CLIENT_OPTS="$NUM_INS $DELAY $SERVER_PORT $MAX_CLIENTS $NUM_SELECTORS $DURATION"
...
# 	 done
 ...
 # 3) After finished test, rename dumps and copy them to a safe location
	 ...
	 ssh tests@${SERVER} mv /work/tests/gameserver/src/out/delay_statistics.txt /work/tests/traces/delay-${COUNTER}-clients-$threadnum-threads.txt
	 
	 echo "Copying cpu trace to storage"
	 ssh tests@${SERVER} mv /work/tests/gameserver/src/out/cpu_statistics.txt /work/tests/traces/cpu-${COUNTER}-clients-$threadnum-threads.txt

	 echo "Copying droped info trace to storage"
	 ssh tests@${SERVER} mv /work/tests/gameserver/src/out/timeouts_statistics.txt  /work/tests/traces/drops-${COUNTER}-clients-$threadnum-threads.txt
